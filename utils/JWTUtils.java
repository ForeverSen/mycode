...
public class JWTUtils {
    ...
    public static final String JWT_SECRET = "aaa";
    ...
    public static String createJWT(Map map, Date expiration) {
        ...
        Instant now = Instant.now();
        String jwt = Jwts.builder().setIssuedAt(Date.from(now)).setClaims(map).setHeader(mapHeahder).setExpiration(expiration).signWith(SignatureAlgorithm.HS256, TextCodec.BASE64.encode(JWT_SECRET)).compact();
    ...
    public static boolean verify(String jwtToken) {
        try {
            ...
            Jwts.parser().setSigningKey(TextCodec.BASE64.encode(JWT_SECRET)).parse(base64jwt);
            return true;
    ...
    public static Map getJWTData(String jwtToken) {
        try {
            byte[] b = Base64.getDecoder().decode(jwtToken);
            String base64jwt = new String(b);
            Jwt jwt = Jwts.parser().setSigningKey(TextCodec.BASE64.encode(JWT_SECRET)).parse(base64jwt);
